...
export function CvButton({ className = "" }: CvButtonProps) {
  ...
  return (
    <a
      href={t.cv.file}
      download
      className={`group relative inline-flex items-center gap-2 overflow-hidden rounded-full border border-accent-to/30 bg-foreground/[0.03] px-6 py-3 text-sm font-semibold text-foreground transition-all hover:scale-[1.03] hover:border-accent-to/60 hover:bg-foreground/[0.05] ${className}`}
    >
